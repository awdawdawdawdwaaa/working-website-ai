...
export default function SingleLightLine({ progress, loaderLineActive = false }) {
  ...
  useFrame(({ camera, clock }) => {
    ...
    // Opacity gate — loader handoff
    if (loaderLineActive) {
      mainMaterial.opacity = 0
      fadeRef.current = null
    } else if (gateActiveRef.current && fadeRef.current === null) {
      fadeRef.current = clock.elapsedTime
    }
    if (!loaderLineActive && fadeRef.current !== null) {
